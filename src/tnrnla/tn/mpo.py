...
class ZpMPO(MPO):
    """
    Z_p charge-conserving MPO wrapper.

    Uses dense tensor storage but enforces an exact zero pattern compatible
    with the local rule

        q_right == q_left + q_out - q_in  (mod p)

    where q_out/q_in are output/input physical charges.
    """
    ...
    def __init__(
        ...
        p=2,
        bond_charges=None,
        phys_out_charges=None,
        phys_in_charges=None,
        net_charge=0,
        enforce=True,
        **kwargs,
    ):
        super().__init__(mpo, **kwargs)
        self._enforce_enabled = True
        self.p = int(p)
        if self.p < 2:
            raise ValueError("p must be >= 2")

        self.net_charge = int(net_charge) % self.p
        self.phys_out_charges = self._normalize_phys_charges(phys_out_charges, out=True)
        self.phys_in_charges = self._normalize_phys_charges(phys_in_charges, out=False)
    ...
    def copy(self):
        out = ZpMPO(
            ...
            phys_out_charges=self.phys_out_charges.copy(),
            phys_in_charges=self.phys_in_charges.copy(),
    ...
    def round(self, stop=Cutoff(1e-14)):
        super().round(stop=stop)
        if getattr(self, "_enforce_enabled", True):
            self.enforce_symmetry()

    def add(self, mpo2, subtract=False, compress=False, stop=Cutoff(1e-14)):
        out = super().add(mpo2, subtract=subtract, compress=compress, stop=stop)
        return ZpMPO.from_mpo(
            out,
            p=self.p,
            bond_charges=None,
            phys_out_charges=self.phys_out_charges.copy(),
    ...
    def _normalize_phys_charges(self, charges, out=True):
        d = int(self[0].shape[0] if out else self[0].shape[2])
        if charges is None:
            q = np.arange(d, dtype=np.int64) % self.p
        else:
            q = np.asarray(charges, dtype=np.int64).ravel() % self.p
            if q.size != d:
                kind = "phys_out_charges" if out else "phys_in_charges"
                raise ValueError(f"{kind} length {q.size} does not match dim {d}")
        return q
    ...
    def _normalize_bond_charges(self, bond_charges):
        ...
        if bond_charges is None:
            out = [np.array([0], dtype=np.int64)]
            for D in dims[1:-1]:
                out.append(np.arange(D, dtype=np.int64) % self.p)
            out.append(np.array([self.net_charge], dtype=np.int64))
            return out
    ...
    def _site_mask(self, site):
        ...
        qO = self.phys_out_charges
        qI = self.phys_in_charges

        if site == 0:
            lhs = (qL[0] + qO[:, None, None] - qI[None, None, :]) % self.p
            return lhs == qR[None, :, None]
        if site == self.N - 1:
            lhs = (qL[:, None, None] + qO[None, :, None] - qI[None, None, :]) % self.p
            return lhs == qR[0]
        lhs = (qL[:, None, None, None] + qO[None, :, None, None] - qI[None, None, None, :]) % self.p
        return lhs == qR[None, None, :, None]
    ...
    def _enforce_site(self, site):
        ...
        if A.shape != mask.shape:
            raise ValueError(f"Site {site} mask shape {mask.shape} does not match tensor shape {A.shape}")
    ...
    def _apply_mps(self, psi):
        if isinstance(psi, ZpMPS):
            # Use standard dense contraction kernels internally, then project
            # back to Zp-structured storage on output.
            mpo_plain = MPO(
                [np.asarray(t).copy() for t in self.tensors],
                orthform=getattr(self, "orthform", "None"),
                rounded=getattr(self, "rounded", False),
                dtype=getattr(self, "dtype", None),
                mv_alg=getattr(self, "_mv_alg", "SRC"),
            )
            mps_plain = MPS(
                [np.asarray(t).copy() for t in psi.tensors],
                orthform=getattr(psi, "orthform", "None"),
                rounded=getattr(psi, "rounded", False),
                dtype=getattr(psi, "dtype", None),
            )
            from .contraction.src import SRC
            out = SRC(mpo_plain, mps_plain, stop=Cutoff(1e-14), finalround=False)
            return ZpMPS.from_mps(
                out,
                p=psi.p,
                bond_charges=None,
                phys_charges=psi.phys_charges.copy(),
                total_charge=(int(psi.total_charge) + int(self.net_charge)) % int(psi.p),
                enforce=False,
            )

        return super()._apply_mps(psi)
    ...
    def from_mpo(
        ...
        *,
        ...
        return cls(
            [np.asarray(t).copy() for t in mpo.tensors],
            p=p,
            bond_charges=bond_charges,
            phys_out_charges=phys_out_charges,
            phys_in_charges=phys_in_charges,
            net_charge=net_charge,
            enforce=enforce,
            orthform=getattr(mpo, "orthform", "None"),
            rounded=getattr(mpo, "rounded", False),
            dtype=getattr(mpo, "dtype", None),
            mv_alg=getattr(mpo, "_mv_alg", "SRC"),
        )
